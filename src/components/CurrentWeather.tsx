...
function CurrentWeather({ isFavorite, data }: CurrentWeatherProps) {
  ...
  const { loading, favoriteWeather } =
    useSelector((state: IinitialProps) => state);
  if (loading) {
  ...
  const handleClick = (data: ApiResponse) => {
    let load: ApiResponse[] = favoriteWeather || [];
    ...
    ()=>dispatch({
      type: "set",
      favoriteWeather: favoritoData,
    });
  };

  const handleRemove = (data: ApiResponse) => {
    let load: ApiResponse[] = favoriteWeather || [];
    const res = load.filter(
